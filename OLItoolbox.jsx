...
function creatingUI( thisObj ){
    ...
    var OLItoolboxDlg = thisObj ;
        var OLItoolboxdlgGroup = OLItoolboxDlg.add( "Group" );
            ...
            var btnsGroupE = OLItoolboxdlgGroup.add( "Group" );
    ...
    var savedEditSortBtnVisibility = JSON.parse( getSavedString( "OLItoolboxSave" , "editSortBtnVisibility" ) );
...
function editSorting(){
    ...
    //Setting the folders to work with.
    var folderToSort = new Folder( "D:/OLIVIA/04_Exports/02_Editing" );
    var sentFolder = new Folder( "D:/OLIVIA/04_Exports/02_Editing/00 - Sent" );
    var toSendFolder = new Folder( "D:/OLIVIA/04_Exports/02_Editing/01 - To Send" );
    var oldVersionsFolder = new Folder( "D:/OLIVIA/04_Exports/02_Editing/00 - Sent/00 - OlderVersions")
    //Getting the videos to Sort and the videos to compare it with.
    var videosToSort = folderToSort.getFiles( "OL-*_comp_*.mov" );
    //Parsing the videos to sort and placing it in the right folder.
    for( var i = 0 ; i < videosToSort.length ; i++ ){
        var currentVid = videosToSort[i];
        var videosSent = sentFolder.getFiles( currentVid.name.slice( 0 , 14 ) + "*" );
        var toErase = true ;
        if( videosSent.length < 1 ){
            toErase = false ;
        } else {
            for( var j = 0 ; j < videosSent.length ; j++ ){
                var comparisionVid = videosSent[j] ;
                var currentVidVersion = currentVid.name.slice( currentVid.name.search( /v[0-9]+/ ) + 1 , currentVid.name.search( /\.[0-9]+/ ) );
                var comparisionVidVersion = comparisionVid.name.slice( comparisionVid.name.search( /v[0-9]+/ ) + 1 , comparisionVid.name.search( /\.[0-9]+/ ) );
                var currentVidSubVersion = currentVid.name.slice( currentVid.name.search( /\.[0-9]+/ ) + 1 , currentVid.name.search( ".mov" ) );
                var comparisionVidSubVersion = comparisionVid.name.slice( comparisionVid.name.search( /\.[0-9]+/ ) + 1 , comparisionVid.name.search( ".mov" ) );
                if( ( currentVidVersion > comparisionVidVersion ) || ( currentVidVersion == comparisionVidVersion && currentVidSubVersion > comparisionVidSubVersion ) ){
                    toErase = false ;
                    comparisionVid.copy( oldVersionsFolder.fsName + "/" + comparisionVid.name );
                    comparisionVid.remove();
                }
            }
        }
        if( toErase ){
            ...
            currentVid.copy( sentFolder.fsName + "/" + currentVid.name );
            currentVid.copy( toSendFolder.fsName + "/" + currentVid.name );
            currentVid.remove();
        }
    }
...
function sortingExports(){
    ...
    var oldVersionsFolder = null ;
    var exports = [] ;
    //Getting the Folder to work in.
    if( ScriptUI.environment.keyboardState.altKey ){
        projectFolder = Folder.myDocuments.selectDlg( "Where is your Folder?" );
        oldVersionsFolder = new Folder( projectFolder.fsName + "/XXXx- Older Versions -xXX" );
        if( !oldVersionsFolder.exists ){ oldVersionsFolder.create(); }
        exports = projectFolder.getFiles( /.+\..+/ );
    } else {
        projectFolder = getSavedString( "OLItoolboxSave" , "OLIFolder" );
        ...
        oldVersionsFolder = new Folder( exportsFolder.fsName + "/XXXx- Older Versions -xXX" );
        if( !oldVersionsFolder.exists ){ oldVersionsFolder.create(); }
    ...
    for( var i = 0 ; i < exports.length ; i++ ){
        ...
        var x = /(.+)_v([0-9]+)\.([0-9]+)(?:%20|)(.+|)\.[a-zA-Z)]{3}$/.exec( currentFile.name );
        if( x == null ){ continue ;}
        var currentFileShotName = x[1];
        var currentFileVersion = x[2];
        var currentFileSubVersion = x[3];
        var title = x[4];
        if( typeof sortedExports[ currentFileShotName ] === "undefined" ){ sortedExports[ currentFileShotName ] = new Array(); }
        if( typeof sortedExports[ currentFileShotName ][ currentFileVersion ] === "undefined" ){ sortedExports[ currentFileShotName ][ currentFileVersion ] = new Array(); }
        if( typeof sortedExports[ currentFileShotName ][ currentFileVersion ][ currentFileSubVersion ] === "undefined" ){ sortedExports[ currentFileShotName ][ currentFileVersion ][ currentFileSubVersion ] = new Array(); }
        if( typeof sortedExports[ currentFileShotName ][ currentFileVersion ][ currentFileSubVersion ][ title ] === "undefined" ){ sortedExports[ currentFileShotName ][ currentFileVersion ][ currentFileSubVersion ][ title ] = new Array(); }
        sortedExports[ currentFileShotName ][ currentFileVersion ][ currentFileSubVersion ][ title ] = currentFile ;
    }
            ...
            if( typeof sortedExports[ shot ][j] !== "undefined" ){
                for( var k = sortedExports[ shot ][j].length ; k >= 0 ; k-- ){
                    if( typeof sortedExports[ shot ][j][k] !== "undefined" ){
                        ...
                        for( x in sortedExports[ shot ][j][k] ){
                            sortedExports[ shot ][j][k][x].copy( oldVersionsFolder.fsName + "/" + sortedExports[ shot ][j][k][x].name );
                            sortedExports[ shot ][j][k][x].remove();
                        }
                    }
                }
            }
...
function convertingTIFFS(){
    ...
    var shotRegExp = new RegExp( /_[0-9]{1,}_[0-9A-Za-z]{2,}_[0-9]{2,}\.TIF/ );
